...
export function extractReadabilityContent(
	doc: Document
...
export async function replaceVariables(
	...
	const regex =
		/{{(?:schema:)?(?:selector:)?(?:selectorHtml:)?(?:prompt:)?\s*([\s\S]*?)\s*}}/g;
	...
	if (matches) {
		for (const match of matches) {
			...
			if (
				normalizedMatch.startsWith("selector:") ||
				normalizedMatch.startsWith("selectorHtml:")
			) {
				const [selectorType, ...selectorParts] =
					normalizedMatch.split(":");
				const selector = selectorParts.join(":").trim();
				const reconstructedMatch = `{{${selectorType}:${selector}}}`;
				replacement = await processSelector(
					tabId,
					reconstructedMatch,
					currentUrl
				);
				...
				const [variableName, ...filterParts] = normalizedMatch
					.split("|")
					.map((part) => part.trim());
...
export function getMetaContent(
	...
	const element = Array.from(doc.querySelectorAll(selector)).find(
		(el) => el.getAttribute(attr)?.toLowerCase() === value.toLowerCase()
	);
...
export async function initializePageContent(
	...
	try {
		...
		if (!readabilityArticle) {
			console.warn(
				"Failed to parse content with Readability, falling back to full content"
			);
		}
		...
		const markdownHighlights = highlights.map((highlight) =>
			createMarkdownContent(highlight.content, currentUrl)
		);
		...
		const currentVariables: { [key: string]: string } = {
			...
			"{{url}}": currentUrl.trim(),
			"{{highlights}}":
				highlights.length > 0 ? JSON.stringify(markdownHighlights) : "",
		};
		...
		if (error instanceof Error) {
			throw new Error(
				`Unable to initialize page content: ${error.message}`
			);
		} else {
...
function getSchemaProperty(
	...
	const searchSchema = (
		...
		if (Array.isArray(data)) {
			...
			if (fullPath === "author.name") {
				return data
					.map((item: any) => searchSchema(item, ["name"], "name"))
					.filter(Boolean)
					.join(", ");
			}
			return data
				.map((item: any) => searchSchema(item, props, fullPath))
				.filter(Boolean)
				.join(", ");
		}
	...
	try {
		const result =
			searchSchema(schemaOrgData, property.split("."), "") ||
			defaultValue;
