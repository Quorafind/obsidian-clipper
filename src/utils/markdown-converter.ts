...
export function createMarkdownContent(content: string, url: string) {
	...
	turndownService.addRule("listItem", {
		...
		replacement: function (
			...
			if (parent instanceof HTMLOListElement) {
				...
				let index =
					Array.from(parent.children).indexOf(node as HTMLElement) +
					1;
		...
		replacement: function (content, node) {
			...
			if (figcaption) {
				...
				const captionMarkdown =
					turndownService.turndown(captionContent);
		...
		return Array.from(mathElements)
			.map((mathElement) => {
				...
				if (alttext) {
					...
					const isInline =
						mathElement.closest(".ltx_eqn_inline") !== null;
					return isInline
						? `$${alttext.trim()}$`
						: `\n$$$\n${alttext.trim()}\n$$$`;
				}
	...
	turndownService.addRule("arXivEnumerate", {
		filter: (node) => {
			return (
				node.nodeName === "OL" &&
				node.classList.contains("ltx_enumerate")
			);
		},
		replacement: function (content, node) {
			if (!(node instanceof HTMLElement)) return content;
		...
		filter: (node: Node): boolean => {
			if (node instanceof HTMLElement) {
				return (
					...
					(node.nodeName === "UL" &&
						node.classList.contains("ltx_biblist")) ||
					(node.nodeName === "OL" &&
						node.parentElement?.classList?.contains("footnotes") ===
							true)
				);
			}
			return false;
		...
		replacement: (content, node) => {
			if (node instanceof HTMLElement) {
				const references = Array.from(node.children).map((li) => {
					...
					const referenceContent = turndownService.turndown(
						li.innerHTML
					);
					// Remove the backlink from the footnote content
					const cleanedContent = referenceContent
						.replace(/\s*↩︎$/, "")
						.trim();
	...
	turndownService.addRule("MathJax", {
		filter: (node) => {
			const isMjxContainer =
				node.nodeName.toLowerCase() === "mjx-container";
