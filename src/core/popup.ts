...
async function loadAndSetupTemplates() {
	...
	const loadedTemplates = await Promise.all(
		(templateIds as string[]).map(async (id: string) => {
			try {
	...
	templates = loadedTemplates.filter(
		(t: Template | null): t is Template => t !== null
	);
		...
		(
			...
			if (request.action === "triggerQuickClip") {
				...
				if (request.isValidUrl) {
					...
					showError(
						"This page cannot be clipped. Only http and https URLs are supported."
					);
				}
...
async function handleClip() {
	...
	if (!isDailyNote) {
		if (!noteNameField || !pathField) {
			showError(
				"Note name or path field is missing. Please try reloading the extension."
			);
			return;
		}
	...
	try {
		if (currentTemplate.behavior === "create") {
			...
			const frontmatter = await memoizedGenerateFrontmatter(
				updatedProperties as Property[]
			);
		...
		const checkProcessing = () => {
			if (!interpretBtn.classList.contains("processing")) {
				if (interpretBtn.textContent?.toLowerCase() === "done") {
					...
					interpretBtn.textContent?.toLowerCase() === "error"
				) {
...
async function refreshFields(
	...
	if (templates.length === 0) {
		...
		showError(
			"No templates available. Please add a template in the settings."
		);
		return;
	}
	...
	try {
		...
		if (!tab.url || isBlankPage(tab.url)) {
			showError(
				"This page cannot be clipped. Please navigate to a web page."
			);
			return;
		}
		if (!isValidUrl(tab.url)) {
		...
		if (extractedData) {
			...
			console.log(extractedData.highlights);
...
async function initializeTemplateFields(
	currentTabId: number,
	...
	for (const property of template.properties) {
		...
		let value = await memoizedReplaceVariables(
			currentTabId!,
		...
		const propertyType =
			generalSettings.propertyTypes.find((p) => p.name === property.name)
				?.type || "text";
		...
		propertyDiv.innerHTML = `
			<div class="metadata-property-key">
				<span class="metadata-property-icon"><i data-lucide="${getPropertyTypeIcon(
			propertyType
		)}"></i></span>
			...
			<div class="metadata-property-value">
				...
				? `<input id="${property.name}" type="checkbox" ${value === "true" ? "checked" : ""
				} data-type="${propertyType}" data-template-value="${escapeHtml(
					property.value
				)}" />`
				: `<input id="${property.name
				}" type="text" value="${escapeHtml(
					value
				)}" data-type="${propertyType}" data-template-value="${escapeHtml(
					property.value
				)}" />`
	...
	if (noteNameField) {
		let formattedNoteName = await memoizedReplaceVariables(
			currentTabId!,
	...
	if (pathField && pathContainer) {
		...
		if (isDailyNote) {
			...
			let formattedPath = await memoizedReplaceVariables(
				currentTabId!,
				template.path,
				variables,
				currentTabId
	...
	if (noteContentField) {
		if (template.noteContentFormat) {
			let content = await memoizedReplaceVariables(
				currentTabId!,
		...
		if (generalSettings.interpreterEnabled) {
			...
			if (
				...
				try {
					...
					console.error(
						"Error auto-processing with interpreter:",
						error
					);
				}
			}
...
async function getReplacedTemplate(
	...
	const replacedTemplate: any = {
		...
		noteNameFormat: await replaceVariables(
			tabId,
			template.noteNameFormat,
			variables,
			currentUrl
		...
		path: template.path,
		noteContentFormat: await replaceVariables(
			tabId,
			template.noteContentFormat,
			variables,
			currentUrl
	...
	if (template.context) {
		replacedTemplate.context = await replaceVariables(
			tabId,
			template.context,
			variables,
			currentUrl
